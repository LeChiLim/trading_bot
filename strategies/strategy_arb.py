...
def run_strategy():
    ...
    while True:
        try:
            ...
            msg_binance = quote_sock_binance.recv(struct.calcsize('!dd16s'), zmq.NOBLOCK)
            bid_binance, ask_binance, symbol_binance = struct.unpack('!dd16s', msg_binance)
            symbol_binance = symbol_binance.decode().strip('\0')

            msg_cryptocoms = quote_sock_cryptocoms.recv(struct.calcsize('!dd16s'), zmq.NOBLOCK)
            bid_cryptocoms, ask_cryptocoms, symbol_cryptocoms = struct.unpack('!dd16s', msg_cryptocoms)
            symbol_cryptocoms = symbol_cryptocoms.decode().strip('\0')

            msg_kraken = quote_sock_kraken.recv(struct.calcsize('!dd16s'), zmq.NOBLOCK)
            bid_kraken, ask_kraken, symbol_kraken = struct.unpack('!dd16s', msg_kraken)
            symbol_kraken = symbol_kraken.decode().strip('\0')

            msg_kucoin = quote_sock_kucoin.recv(struct.calcsize('!dd16s'), zmq.NOBLOCK)
            bid_kucoin, ask_kucoin, symbol_kucoin = struct.unpack('!dd16s', msg_kucoin)
            symbol_kucoin = symbol_kucoin.decode().strip('\0')
